...
void setup() {
  ...
  oldminutes = 100;
  displayTime(timeClient.getHours(), timeClient.getMinutes(), summerTime, timeClient.getEpochTime());
}
...
void displayTime(int hours, int minutes, int summerTime, unsigned long epochTime) {
  struct tm* ti;
...
<body>
  ...
  <div class='button-container'>
    <label for="summerTimeSlider">Summer Time:</label>
    <input type="range" id="summerTimeSlider" min="1" max="0" value="0" step="1">
    <span id="summerTimeValue">Winter Time</span>
  </div>
  <div class='button-container'>
  ...
  <form id="colorForm" style="display:none;">
    ...
    <input type="text" id="summerTime" name="summerTime">
  </form>
  <script>
    ...
    const summerTimeSlider = document.getElementById('summerTimeSlider');
    const summerTimeValue = document.getElementById('summerTimeValue');
    ...
    summerTimeSlider.addEventListener('input', function() {
      summerTimeValue.textContent = summerTimeSlider.value == 1 ? 'Summer Time' : 'Winter Time';
    });
    ...
    function submitColor(endpoint) {
      ...
      document.getElementById('summerTime').value = summerTimeSlider.value;
...
void handleApply() {
  if (server.hasArg("startTimeValue") && server.hasArg("endTimeValue") && server.hasArg("summerTime")) {
    Serial.println("apply");

    String startTimeStr = server.arg("startTimeValue");
    String endTimeStr = server.arg("endTimeValue");
    int summerTimeValue = server.arg("summerTime").toInt();
    ...
    summerTime = summerTimeValue; // Speichere die Sommerzeit-Einstellung

    Serial.printf("Time values - TimeOnHour: %d, TimeOnMinute: %d, TimeOffHour: %d, TimeOffMinute: %d, SummerTime: %d\n", TimeOnHour, TimeOnMinute, TimeOffHour, TimeOffMinute, summerTime);
    saveColorsToEEPROM();
    oldminutes = 100;
    server.sendHeader("Location", "/", true);
    server.send(303, "text/plain", "Redirecting to home");
